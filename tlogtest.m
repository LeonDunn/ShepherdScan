function tlogtest(app,filename, OutFolder)
...
%return
%extract the jaws and MLC whilst the beam is on.
for i = 1:size(snapShotFinal,1) %beam 
    for j = 1:size(snapShotFinal,2) %dample
            CollAngle(i,1) = snapShotFinal(j).colRotationA;
...
%can we downsample the Mu_seg?
%MU_seg = interp1(1:length(MU_seg),)
...
%                     end
    ...
    writematrix(final_fluence_beam{k},strcat(OutFolder,fname(1,1:end-3),'.csv'));
    figure(k)
