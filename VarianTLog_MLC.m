%this program reads in and derives a Varian Trajectory log fluence map
%leon Dunn - Beginning testing 16/2/2023
% clear all
% close all
% %get the dicom file from a folder
%  [input_film,pathname] = uigetfile('*.bin','DICOM PLAN FILE (*.*)', ...
%            'Select files', ... 
%           'MultiSelect', 'on');
...
function eFluenceQA_MMLC(fileNameTDS)
fid=fopen(fileNameTDS);
disp(fid)
